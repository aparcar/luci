...
ubus_counter = 0;
ubus_closed = 0;
function ubus_call(command, argument, params, variable, async) {
	if (async == undefined) {
		async = true;
	}
	var request_data = {};
	...
	ubus_counter++;
	var request = new XMLHttpRequest();
	request.open("POST", ubus_url, async);
	request.setRequestHeader("Content-type", "application/json");
...
function upgrade_request() {
	...
	var request_dict = {}
	request_dict.distro = data.release.distribution.toLowerCase();
	request_dict.target = data.release.target.split("\/")[0];
	request_dict.subtarget = data.release.target.split("\/")[1];
	...
	console.log(request_dict.packages)
	ubus_call("checksum", "checksum", { "type": "sha256", "data": request_dict.packages.sort().join(" ") }, "checksum", false)
	console.log("pkg_hash " + data.checksum)
	pkg_hash = data.checksum.substring(0, 12)
	...
	ubus_call("checksum", "checksum", { "type": "sha256", "data": hash_string }, "checksum", false)
	request_hash = data.checksum.substring(0, 12)
